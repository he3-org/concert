...
1. `docs/concert/state.json` — current stage, mission_path, workflow_path, pipeline status
2. `docs/concert/stage-registry.jsonc` — stage definitions, ordering, agent mappings
...
2. **Read** `docs/concert/stage-registry.jsonc` → get stage ordering and agent mappings.
