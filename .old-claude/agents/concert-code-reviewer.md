...
<skills>
Read `.claude/skills/concert-core/SKILL.md` for: boot sequence, state management, next_action protocol, severity classification, commit conventions.
</skills>
...
1. `.concert/state.json` — current stage, execution context
...
6. **Update state.json** with review result. Write `next_action`:
   - If PASS or only MIN/NTH → `{ type: "run_agent", target: "concert-coder", message: "Ready for next task" }`
   - If FAIL (CRIT/MAJ) → `{ type: "run_agent", target: "concert-coder", message: "Revision needed — see findings" }`
