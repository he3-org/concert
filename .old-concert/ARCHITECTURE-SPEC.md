...
2. Check for existing Concert installation:
   a. If .concert/ exists → warn, ask to overwrite or abort
   b. If concert.jsonc exists → warn, ask to merge or abort
3. Copy templates/ → cwd recursively
4. Create .concert/state.json with empty schema
...
Concert workflow definitions (`.md` files in `.concert/workflows/`) are the design-layer source of truth. During `init` and `update`, the npm package generates:
...
- **Concert workflows** tell agents _what to do_ (read by Claude at runtime)
- **GitHub workflows** tell GitHub Actions _when to trigger_ (read by GitHub's CI system)
...
| User content (mission files)  | Never touched      | Inside `.concert/missions/`             |
...
**Chose:** Single `.concert/state.json` for all state
