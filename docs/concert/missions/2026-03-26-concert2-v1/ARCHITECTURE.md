...
4. If there are staged changes:
   a. Commit with message: "wip: concert-push handoff"
...
interface ConcertState {
  ...
  status_display: string;             // "wip_pr" | "status_md"
...
Concert 1 manually maintains two GitHub workflow files:
- `.github/workflows/concert-auto-continue.yml` — triggers on state.json pushes
- `.github/workflows/concert-version-check.yml` — weekly version check (currently references Go)

And manually maintains GitHub agent stubs in `.github/agents/`.
...
|-----------|------|-----------|
| Task file has ≤3 tasks | Simple (single runner) | Low complexity, not worth subagent overhead |
| Task file has >3 tasks | Full loop (orchestrator/coder/reviewer) | Higher complexity benefits from formal review |
| GitHub Agents UI | Simple only | GitHub Agents UI does not support subagent spawning |
| Claude Code | Either, based on task count | Supports Task tool for subagent spawning |

**Trade-off:** The ≤3 task threshold is a heuristic. It trades review quality for speed on simple tasks. If telemetry shows simple-mode tasks have higher failure rates, the threshold can be lowered or removed.

---

## 9. Cross-Environment Handoff
...
Claude Code session (plan exhausted)
  │
  ├── Context gating fires (>85% context or plan limit)
  ├── Current task completes fully (never interrupts mid-task)
  ├── State.json updated with full execution position
  ├── quality_loop_state saved if mid-loop
  │
  ▼
...
## 10. Context Usage Gating

### Where It Runs

Context gating runs at **task boundaries only** — after a task completes and before the next task starts. It never interrupts a task mid-execution or mid-quality-loop.

### How It Works

```
Before each task:
  1. Check if running in Claude Code (GitHub manages its own limits)
  2. Read execution.max_context_usage_percent from concert.jsonc (default: 85)
  3. Estimate current context usage
  4. If usage > threshold:
     a. Save all state (including quality_loop_state if active)
     b. Commit state.json
     c. Output handoff instructions
     d. Stop execution
  5. If usage cannot be determined:
     a. Log warning
     b. Proceed (fail-open)
```
...
Claude Code does not expose an exact context usage API. The orchestrator estimates usage based on:
- Number of files read in the current session
- Number of subagent spawns
- Rough token counting of conversation history

This is an approximation. The threshold (85% default) includes a safety margin — the goal is to stop early enough that `concert push` and handoff instructions can be output before hitting the hard limit.

**Trade-off:** Fail-open (proceed when usage is unknown) risks mid-task exhaustion but avoids false positives that would stop execution unnecessarily. Since state is committed after every task, the worst case is losing one task's work.

---

## 11. Model Tier Routing
...
// concert.jsonc
{
  "model_tiers": {
    ...
    "github": {
      "haiku": null,           // Promoted to sonnet (same price on GitHub)
      "sonnet": "claude-sonnet-4-6",
      "opus": "claude-opus-4-6"
    }
  }
...
2. Detect current environment (Claude Code or GitHub)
3. Look up model_tiers[environment][model]
4. If null (e.g., haiku on GitHub) → promote to next tier (sonnet)
5. Spawn subagent / execute with resolved model
```

### Tier Promotion

When a tier is unavailable in the current environment, promote upward:
- haiku → sonnet (when haiku is null/unavailable)
- sonnet → opus (unlikely, but supported)

Promotion is logged in telemetry so the cost report can distinguish planned vs. promoted model usage.
...
## 17. Migration from Concert 1

Concert 2's npm package replaces Concert 1's Go binary. The migration path:

1. User runs `npx @he3-org/concert init` in an existing Concert 1 repo
2. Init detects existing Concert files and offers to update
3. Managed files (agents, workflows, skills, GitHub agents, GitHub workflows) are overwritten with v2 versions
4. `concert.jsonc` is surgically merged — user's project-specific values preserved
5. `state.json` is surgically merged — active mission data preserved, new schema fields added
6. The Go-based version-check workflow is replaced with an npm-based one
7. Any active mission can continue with Concert 2 agents via `concert-continue`

No data loss. No mission restart required. The migration is a superset of the normal `update` operation.
